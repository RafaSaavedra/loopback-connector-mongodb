...
MongoDB.prototype.fromDatabase = function (model, data) {
  ...
  for (var p in props) {
    ...
    } else if(prop && prop.type === GeoPoint) {
      data[p] = new GeoPoint({ lat: data[p].coordinates[1], lng: data[p].coordinates[0] })
    }
  }
  return data;
};
...
MongoDB.prototype.toDatabase = function (model, data) {
  ...
  for (var p in props) {
    var prop = props[p];
    if(prop && prop.type === GeoPoint) {
      data[p] = { coordinates: [data[p].lng, data[p].lat], type: 'Point' };
    }
  }
...
MongoDB.prototype.create = function (model, data, callback) {
  ...
  this.collection(model).insert(data, {safe: true}, function (err, m) {
    ...
    //console.log('collection(model).insert result: ', m);
    //idValue = m[0]._id;
    idValue = m.ops[0]._id;
  ...
  Object.keys(where).forEach(function (k) {
    ...
    if (spec) {
      if (spec === 'between') {
        ...
        query[k] = {$regex: newRegExp(cond)};
      } else if (spec === 'nlike') {
        query[k] = {$not: newRegExp(cond)};
      } else if (spec === 'neq') {
        query[k] = {$ne: cond};
      } else if (spec === '$near') {
        query[k] = {};
